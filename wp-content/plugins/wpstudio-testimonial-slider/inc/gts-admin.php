...
class WPSTUDIO_GTS_Settings extends Genesis_Admin_Boxes {
	...
	function gsw_settings() {
		...
		<p>
			<label style="width: 180px; margin: 0 40px 0 0; font-weight: bold; display: inline-block;" for="<?php echo $this->get_field_name( 'gts_order' ); ?>"><?php _e( 'Show random:', 'gts-widget' ); ?></label>
			<select name="<?php echo $this->get_field_name( 'gts_order' ); ?>">
				<?php
				$positions = array( 'yes', 'no' );
				foreach ( $positions as $position ) {
					echo '<option value="' . $position . '"' . selected( $this->get_field_value( 'gts_order' ), '' . $position ) . '>' . $position . '</option>';
				}
				?>
			</select>
